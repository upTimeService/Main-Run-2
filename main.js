  ...
  login(loginData, async (err, api) => {
    ...
    (global.config.version = config.version),
      (async () => {
        ...
        console.log(cv(`\n` + ` ★ [TANVIR-143-BOT] COMMMANDS LOADING...`));
...
global.client.commands.set(config.name, module);
            ...
            global.loading.log(
              `${cra(`INSTALLED`)} ${cb(config.name)}.js`,
              "TANVIR-143-BOT",
            );
          } catch (error) {
            global.loading.err(
              `${chalk.hex("#ff7100")(`RUNNING`)} ${chalk.hex("#FFFF00")(command)} WRONG ` +
                error,
              "TANVIR-143-BOT",
            );
          }
    ...
    global.handleListen = api.listenMqtt(async (error, event) => {
      if (error) {
        ...
        if (error.error === "Not logged in") {
          logger.log(
            "BOT Account is Locked, unlock now or appstate past and run",
            "CHECKPOINT",
          );
          return process.exit(0);
        }
...
(async () => {
  try {
    console.log(cv(`\n` + `★ [TANVIR-143-BOT] DATABASE ★`));
    global.loading.log(
      `${cra(`[ CONNECT ]`)} Connected database successfully!`,
      "DATABASE",
    );
    onBot();
  } catch (error) {
