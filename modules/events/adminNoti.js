    ...
    switch (logMessageType) {
      case "log:thread-admins": {
        if (logMessageData.ADMIN_EVENT === "add_admin") {
          ...
          api.sendMessage(`\n❯${Users.getNameUser(logMessageData.TARGET_ID)} Became a group admin`, threadID);
        } else if (logMessageData.ADMIN_EVENT === "remove_admin") {
          dataThread.adminIDs = dataThread.adminIDs.filter(item => item.id !== logMessageData.TARGET_ID);
          api.sendMessage(`❯ Remove user's admin position ${logMessageData.TARGET_ID}`, threadID);
        }
        break;
      }
      case "log:user-nickname": {
        ...
        if (participant_id && nickname) {
          ...
          api.sendMessage(`❯ Updated nickname for ${participantName}: ${formattedNickname}.`, threadID);
        }
        break;
      }
      case "log:thread-name": {
        dataThread.threadName = logMessageData.name || null;
        api.sendMessage(`n❯ ${(dataThread.threadName) ? `Updated Group Name to: ${dataThread.threadName}` : 'Cleared the Group Name'}.`, threadID);
        break;
      }
      ...
      case "log:thread-call": {
        if (logMessageData.event === "group_call_started") {
          ...
          api.sendMessage(`[🤍]  ${name} STARTED A ${(logMessageData.video) ? 'VIDEO ' : ''}CALL.`, threadID);
          ...
          api.sendMessage(`❯ ${(logMessageData.video) ? 'Video' : ''} কল কেটে দেওয়া হয়েছে\n❯কল চলেছে: ${timeFormat}`, threadID);
